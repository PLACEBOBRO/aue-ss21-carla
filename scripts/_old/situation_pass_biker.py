...
def spawnBike(spawn_point):
    batch = []
    print("SPAWNING ONE BIKE...")
    blueprint = random.choice(blueprints.filter("vehicle.bh.crossbike"))
...
def onPress(key):
    try:
        ...
        if str(key) == 'Key.f1':
            spawnBike(176)
            # Possibility to spawn second Bike at location 44
